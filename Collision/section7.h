...
struct Node {
    Vector3 center;     // Center point of octree node (not strictly needed)
...
Node *BuildOctree(Vector3 center, float halfWidth, int stopDepth)
{
...
void VisitOverlappedNodes(KDNode *pNode, Sphere *s, Vector3 &volNearPt)
{
...
void VisitNodes(KDNode *pNode, Vector3 a, Vector d, float tmax)
{
    if (pNode == NULL) return;
...
void SortAndSweepAABBArray(void)
{
    ...
    for (int i = 0; i < MAX_OBJECTS; i++) {
        ...
        Vector3 p = 0.5f * (gAABBArray[i]->min + gAABBArray[i]->max);
