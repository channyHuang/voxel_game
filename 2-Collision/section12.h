...
Vector3 vertex[MAX_VERTICES]; // unique vertices within tolerance
int numVertices;            // number of unique vertices currently stored

int LocateVertexInBucket(Vector3 v, unsigned int bucket, Vector3 **weldVertex) {
...
void AddVertexToBucket(Vector3 v, unsigned int bucket)
{
...
void WeldVertices(Vector3 v[], int n) {
...
struct HalfEdge {
    HalfEdge *ht;   // The matching 搕win?half-edge of the opposing face
...
struct WingedTriangle {
    ...
    // be stored 揻or free?inside the triangle pointers
...
void NewellPlane(int n, Vector3 v[], Plane *p)
{
    ...
    p->d = Dot(centroid, p->n) / n; // 揷entroid / n?is the true centroid point
}



// Test if n-gon specified by vertices v[] is planar
int IsPlanar(int n, Vector3 v[])
{
...
void Triangulate(Vector3 v[], int n)
{
    ...
    while (n > 3) {
        ...
        if (TriangleIsCCW(v[prev[i]], v[i], v[next[i]])) {
            ...
            // The 慹ar?triangle is clockwise so v[i] is not an ear
            isEar = 0;
        }
        ...
        if (isEar) {
            ...
            // 慏elete?vertex v[i] by redirecting next and previous links
...
Mesh HertelMehlhorn(Polygon p)
{
    ...
    for (int i = 0; i < numDiagonals; i++) {
        // Test if the i抰h diagonal can be removed without creating
