...
bool ObjFileLoader::loadObjFile(std::string sObjFile, FileObject &stFileObject) {
    ...
    while (std::getline(ifs, value)) {
        ...
        switch (value[0]) {
        case 'f':
            sscanf(value.c_str(), "%s %f/%f/%f %f/%f/%f %f/%f/%f", &tmp, &veci[0][0], &veci[0][1], &veci[0][2], &veci[1][0], &veci[1][1], &veci[1][2], &veci[2][0], &veci[2][1], &veci[2][2]);
...
bool ObjFileLoader::pushToData(FileObject &stFileObject) {
    ...
    for (size_t i = 0; i < stFileObject.faces.size(); ++i) {
        ...
        *p++ = stFileObject.normals[stFileObject.faces[i][0]].x();
        *p++ = stFileObject.normals[stFileObject.faces[i][1]].y();
        *p++ = stFileObject.normals[stFileObject.faces[i][2]].z();
        m_count += 6;
    }
    return true;
}
